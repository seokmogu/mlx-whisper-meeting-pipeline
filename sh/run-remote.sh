...
for proj in "${PROJECTS[@]}"; do
  find "$LOCAL_BASE/notes/$proj" -maxdepth 1 -type f -name '*.md' 2>/dev/null || true
done | sort > "$NOTES_BEFORE"
...
for proj in "${PROJECTS[@]}"; do
  find "$LOCAL_BASE/notes/$proj" -maxdepth 1 -type f -name '*.md' 2>/dev/null || true
done | sort > "$NOTES_AFTER"
comm -13 "$NOTES_BEFORE" "$NOTES_AFTER" > "$NOTES_NEW"
if [ -s "$NOTES_NEW" ] && [ -n "${NOTION_UPLOAD_DATABASE_ID:-}" ]; then
  cat "$NOTES_NEW" >> "$UPLOAD_PENDING"
...
elif [ -z "${NOTION_UPLOAD_DATABASE_ID:-}" ]; then
